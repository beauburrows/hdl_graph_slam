...
class BagPlayer:
	def __init__(self, bagfile, start, duration):
		...
		print 'ok, playing. done with init'
		self.play()
	...
	def play_realtime(self, duration):
		...
		stamp_wall = time.time() * 0.2 # TIME SLOW ################################################
	...
	def play(self):
		...
		topic, msg, stamp = self.play_realtime(rospy.Duration(10000))
		self.update_time_subs()
		...
		try:
			while not rospy.is_shutdown():
				...
				self.clock_pub.publish(clock_msg)
				self.publishers[topic].publish(msg)
				#print 'hitting next on message_generator'
				topic, msg, stamp = self.message_generator.next()
		except:
